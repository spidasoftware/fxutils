 ...
 * Initially copied verbatim from com.sun.javafx.scene.control.behavior.TextAreaBehavior.
 * Functional changes were made in mousePressed() and mouseDragged().
 */
...
//            setCaretOpacity(if (textInputControl.dot == textInputControl.mark) then 1.0 else 0.0);
			...
			if (e.getButton() == MouseButton.PRIMARY && !(e.isMiddleButtonDown() || e.isSecondaryButtonDown())) {
				...
				// SPIDA functional change: remove e.isSynthesized() check so the first else-if will position the caret on finger press
...
//                    displaySoftwareKeyboard(true);
	...
	@Override public void mouseDragged(MouseEvent e) {
		final TextArea textArea = getNode();
		// SPIDA functional change: revert JDK bugfix to re-add !deferClick check so the end-caret is updated whenever the cursor is moved by touch
