...
/**
 * This is an overridden JavaFX class with custom bugfixes.
 * Initially copied verbatim from com.sun.javafx.scene.control.skin.TextAreaSkin.
 * Non-functional changes were made to make it compile including helper methods
 * to access package-private members in the superclass using reflection.
 *  @see TextArea
 *  @since 9
 *  Look at the overridden TextAreaBehavior for functional changes.
 */
public class TextAreaSkin extends TextInputControlSkin<TextArea> {
	...
	private TextAreaSkin.ContentView contentView = new TextAreaSkin.ContentView();
...
//        setManaged(false);
		...
		// Add caret
		getCaretPath().setManaged(false);
		getCaretPath().setStrokeWidth(1);
		getCaretPath().fillProperty().bind(textFillProperty());
		getCaretPath().strokeProperty().bind(textFillProperty());
		// modifying visibility of the caret forces a layout-pass (RT-32373), so
		// instead we modify the opacity.
		getCaretPath().opacityProperty().bind(new DoubleBinding() {
			{ bind(caretVisibleProperty()); }
			@Override protected double computeValue() {
				return caretVisibleProperty().get() ? 1.0 : 0.0;
			}
		});
		contentView.getChildren().add(getCaretPath());

		if (getSHOW_HANDLES()) {
			contentView.getChildren().addAll(getCaretHandle(), getSelectionHandle1(), getSelectionHandle2());
		}
		...
		updateFontMetrics();
		getFontMetrics().addListener(valueModel -> {
			updateFontMetrics();
		});
		...
		if (getSHOW_HANDLES()) {
			getSelectionHandle1().setRotate(180);
			...
			getCaretHandle().setOnMousePressed(handlePressHandler);
			getSelectionHandle1().setOnMousePressed(handlePressHandler);
			getSelectionHandle2().setOnMousePressed(handlePressHandler);

			getCaretHandle().setOnMouseReleased(handleReleaseHandler);
			getSelectionHandle1().setOnMouseReleased(handleReleaseHandler);
			getSelectionHandle2().setOnMouseReleased(handleReleaseHandler);

			getCaretHandle().setOnMouseDragged(e -> {
				Text textNode = getTextNode();
				Point2D tp = textNode.localToScene(0, 0);
				Point2D p = new Point2D(e.getSceneX() - tp.getX() - pressX + getCaretHandle().getWidth() / 2,
			...
			getSelectionHandle1().setOnMouseDragged(e -> {
				TextArea control1 = getSkinnable();
				Text textNode = getTextNode();
				Point2D tp = textNode.localToScene(0, 0);
				Point2D p = new Point2D(e.getSceneX() - tp.getX() - pressX + getSelectionHandle1().getWidth() / 2,
						e.getSceneY() - tp.getY() - pressY + getSelectionHandle1().getHeight() + 5);
				HitInfo hit = textNode.hitTest(translateCaretPosition(p));
			...
			getSelectionHandle2().setOnMouseDragged(e -> {
				TextArea control1 = getSkinnable();
				Text textNode = getTextNode();
				Point2D tp = textNode.localToScene(0, 0);
				Point2D p = new Point2D(e.getSceneX() - tp.getX() - pressX + getSelectionHandle2().getWidth() / 2,
						e.getSceneY() - tp.getY() - pressY - 6);
				HitInfo hit = textNode.hitTest(translateCaretPosition(p));
				if (control1.getAnchor() > control1.getCaretPosition()) {
					// Swap caret and anchor
	...
	/**************************************************************************
	 *
	 * Reflection methods to access package-private superclass members
	 *
	 **************************************************************************/
	protected Path getCaretPath() {
		return (Path) getSuperPrivateField("caretPath");
	}

	protected ObservableBooleanValue caretVisibleProperty() {
		return (ObservableBooleanValue) getSuperPrivateGetter("caretVisibleProperty");
	}

	protected boolean getSHOW_HANDLES() {
		return (boolean) getSuperPrivateField("SHOW_HANDLES");
	}

	protected StackPane getSelectionHandle1() {
		return (StackPane) getSuperPrivateField("selectionHandle1");
	}

	protected StackPane getSelectionHandle2() {
		return (StackPane) getSuperPrivateField("selectionHandle2");
	}

	protected ObservableObjectValue<FontMetrics> getFontMetrics() {
		return (ObservableObjectValue<FontMetrics>) getSuperPrivateField("fontMetrics");
	}

	protected StackPane getCaretHandle() {
		return (StackPane) getSuperPrivateField("caretHandle");
	}

	protected boolean isRTL() {
		return (boolean) getSuperPrivateGetter("isRTL");
	}

	protected Object getSuperPrivateField(String fieldName) {
		Class clazz = super.getClass().getSuperclass();
		try {
			return ReflectionUtils.getPrivateFieldValue(clazz, this, fieldName);
		} catch (ReflectionException | InvocationTargetException e) {
			e.printStackTrace();
			return null;
		}
	}

	protected Object getSuperPrivateGetter(String methodName) {
		try {
			Class clazz = super.getClass().getSuperclass();
			return ReflectionUtils.executePrivateMethod(clazz, this, methodName);
		} catch (InvocationTargetException | ReflectionException e) {
			e.printStackTrace();
			return null;
		}
	}
	...
	private void nextCharacterVisually(boolean moveRight) {
		...
		Text textNode = getTextNode();
		Bounds caretBounds = getCaretPath().getLayoutBounds();
		if (getCaretPath().getElements().size() == 4) {
			// The caret is split
			// TODO: Find a better way to get the primary caret position
			// instead of depending on the internal implementation.
			// See RT-25465.
			caretBounds = new Path(getCaretPath().getElements().get(0), getCaretPath().getElements().get(1)).getLayoutBounds();
		}
	...
	public Bounds getCaretBounds() {
		return getSkinnable().sceneToLocal(getCaretPath().localToScene(getCaretPath().getBoundsInLocal()));
	}
	...
	private void scrollCaretToVisible() {
		TextArea textArea = getSkinnable();
		Bounds bounds = getCaretPath().getLayoutBounds();
		...
		if (getSHOW_HANDLES()) {
			if (getCaretHandle().isVisible()) {
				h += getCaretHandle().getHeight();
			} else if (getSelectionHandle1().isVisible() && getSelectionHandle2().isVisible()) {
				x -= getSelectionHandle1().getWidth() / 2;
				y -= getSelectionHandle1().getHeight();
				w += getSelectionHandle1().getWidth() / 2 + getSelectionHandle2().getWidth() / 2;
				h += getSelectionHandle1().getHeight() + getSelectionHandle2().getHeight();
			}
		}
	...
	private void updateFontMetrics() {
		...
		characterWidth = getFontMetrics().get().getCharWidth('W');
	}
	...
	private class ContentView extends Region {
		...
		@Override public void layoutChildren() {
			...
			Bounds oldCaretBounds = getCaretPath().getBoundsInParent();
			...
			if (getSHOW_HANDLES()) {
				...
				if (selection.getLength() > 0) {
					getSelectionHandle1().resize(getSelectionHandle1().prefWidth(-1),
							getSelectionHandle1().prefHeight(-1));
					getSelectionHandle2().resize(getSelectionHandle2().prefWidth(-1),
							getSelectionHandle2().prefHeight(-1));
				} else {
					getCaretHandle().resize(getCaretHandle().prefWidth(-1),
							getCaretHandle().prefHeight(-1));
				}
				...
				if (selection.getLength() > 0) {
					...
					getCaretPath().getElements().clear();
					getCaretPath().getElements().addAll(paragraphNode.getCaretShape());
					getCaretPath().setLayoutX(paragraphNode.getLayoutX());
					getCaretPath().setLayoutY(paragraphNode.getLayoutY());

					Bounds b = getCaretPath().getBoundsInParent();
					if (caretPos < anchorPos) {
						getSelectionHandle2().setLayoutX(b.getMinX() - getSelectionHandle2().getWidth() / 2);
						getSelectionHandle2().setLayoutY(b.getMaxY() - 1);
					} else {
						getSelectionHandle1().setLayoutX(b.getMinX() - getSelectionHandle1().getWidth() / 2);
						getSelectionHandle1().setLayoutY(b.getMinY() - getSelectionHandle1().getHeight() + 1);
					}
				}
			}

			{
				...
				getCaretPath().setLayoutY(paragraphNode.getLayoutY());
				if (oldCaretBounds == null || !oldCaretBounds.equals(getCaretPath().getBoundsInParent())) {
			...
			if (getSHOW_HANDLES()) {
				...
				if (selection.getLength() > 0) {
					...
					getCaretHandle().setLayoutX(b.getMinX() - getCaretHandle().getWidth() / 2 + 1);
					getCaretHandle().setLayoutY(b.getMaxY());
				}
			}
